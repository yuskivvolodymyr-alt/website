/**
 * QubeNode Wallet Connector - Enhanced with MetaMask Support
 * IMPORTANT: This version ADDS MetaMask support WITHOUT breaking existing Keplr/Cosmostation functionality
 */
...
class QubeNodeWalletConnector {
    ...
    init() {
        console.log('🚀 QubeNode Wallet Connector initialized (with MetaMask support)');
    ...
    setupEventListeners() {
        ...
        // Wallet buttons - Keplr and Cosmostation (existing)
        ...
        console.log('✅ Event listeners attached (including MetaMask)');
    }
    ...
    // ==================== COSMOSTATION CONNECTION (UNCHANGED) ====================
    ...
    // ==================== METAMASK CONNECTION (NEW) ====================
    async connectMetaMask() {
        console.log('🦊 Connecting MetaMask...');
        
        if (!window.MetaMaskConnector) {
            console.error('MetaMask connector not loaded');
            alert('MetaMask connector не завантажено. Будь ласка, перезавантажте сторінку.');
            return;
        }
        
        this.showConnecting('modalMetaMaskBtn');
        ...
        try {
            ...
            // Connect to MetaMask
            const result = await this.metamaskConnector.connect();
            ...
            alert('Помилка підключення MetaMask: ' + error.message);
            this.resetButton('modalMetaMaskBtn', '🦊', 'MetaMask');
        }
    }
    ...
    disconnect() {
        // Disconnect MetaMask if it was connected
        if (this.walletType === 'metamask' && this.metamaskConnector) {
            this.metamaskConnector.disconnect();
            this.metamaskConnector = null;
        }
    ...
    getMetaMaskConnector() { return this.metamaskConnector; }
}

// Initialize on DOM ready
document.addEventListener('DOMContentLoaded', function() {
    console.log('📦 Initializing QubeNode Wallet Connector (with MetaMask)...');
